...
Plot changes (V12b — style-matched to reference image):
  1. Navy blue (#000080) for parallax/1/plx traces everywhere.
  2. Median lines are DOTTED (':') for both green and navy blue.
  3. MAD shaded bands very faint; edge lines THIN DOTTED.
  4. Red/crimson "all matched" histogram visible dashed, behind green.
  5. Reference distance band: solid black line + very faint gray fill
     with DOTTED black edge lines.
  ...
  7. Summary: 4 separate paginated series (Dist, RV, PM, RADEC).
     ...
     — Per-panel legends + Med/MAD stat boxes (blue left, green right)
     — No KS p-value text
     — Gold filter: >=15 green; per-type must have >=5 passing,
       else relax to >=10 green for that type.
  8. Grand summary updated accordingly.
  9. Pickle-based result caching: --replot flag skips analysis,
     loads saved results, and only re-generates plots.
 10. Reference bands: GC from literature (GC_dist.csv), OC from parallax,
     SGR from bin centers, DW from catalog metadata.
Author: Sutirtha (V12b-plots, publication-grade, style-matched)
================================================================================
"""
...
class Config:
    ...
    SUMMARY_MIN_HIGH_PMEM=15
    SUMMARY_MIN_HIGH_PMEM_RELAX=10
    SUMMARY_MIN_PER_TYPE=5
    SUMMARY_KS_ALPHA=0.05
    CMAP_PMEM='RdYlGn'; PLOT_DPI=150; SAVE_FORMAT='png'
    # ── V12b colour palette (style-matched to reference image) ──
    COL_MASTER='#DC143C'       # Crimson for all-matched
    COL_MASTER_LW=1.5          # Red step line width
    COL_MASTER_ALPHA=0.55      # Red visible but not dominant
    COL_MEMBER='#000080'       # Navy blue (member / parallax)
    COL_HIGHMEM='#006400'      # Dark green (high P_mem)
    COL_PLX='#000080'          # Navy blue parallax
    COL_GRAY_BG='#C0C0C0'
    GRAY_BG_ALPHA=1.0; GRAY_BG_SIZE=6
    # V12b: Median & MAD styling — matched to reference image
    MEDIAN_LS=':'              # DOTTED median lines (as in ref image)
    MEDIAN_LW=2.0
    MAD_ALPHA=0.06             # Very faint MAD band fill
    MAD_EDGE_ALPHA=0.35        # Thin sleek MAD edge lines
    MAD_EDGE_LW=0.7            # Thin MAD band border linewidth
    MAD_EDGE_LS=':'            # DOTTED MAD edge lines (sleek classic)
    # V12b: Reference band styling — dotted edges, faint fill
    REF_LW=2.5; REF_ALPHA=0.95
    REF_BAND_ALPHA=0.10        # Faint classic ref band fill
    REF_BAND_EDGE_ALPHA=0.55   # Visible but not heavy dotted edge
    REF_BAND_EDGE_LW=1.0       # Ref band border linewidth
    REF_BAND_EDGE_LS=':'       # DOTTED ref band edge lines
    # V12b: Histogram step styling
    STEP_LW_GREEN=2.2          # Green step line width
    STEP_LW_BLUE=2.0           # Blue step line width
    KDE_LW=1.5                 # KDE overlay line width
    KDE_LS='--'                # KDE as dashed (as in ref image)
    KDE_ALPHA=0.7
    # V12b: Histogram zorder control — GREEN ON TOP
    ZORDER_RED=3; ZORDER_PLX=5; ZORDER_GREEN=7; ZORDER_REF=20
    # V12b: Summary panel grid
    SUMMARY_NCOLS=6; SUMMARY_NROWS=6
    SUMMARY_PANEL_W=4.2; SUMMARY_PANEL_H=4.2
...
def _extract_gc_name_variants(name):
    if not isinstance(name, str): name = str(name)
    base = normalize_name(name)
    variants = {base}
    parts = re.split(r'[_\s\-]+', name.strip())
    catalog_pattern = re.compile(r'^(NGC|IC|Pal|Ter|Rup|BH|ESO|FSR|AM|HP|Ton|UKS|VVV|Arp|Djor|Lynga|Mercer|Whiting|Pyxis|Crater|Eridanus|Laevens|Liller|RLGC|Sgr|2MASS)$', re.IGNORECASE)
    i = 0
    while i < len(parts):
        p = parts[i]
        if catalog_pattern.match(p) and i+1 < len(parts):
            try:
                sub = p + parts[i+1]
                variants.add(normalize_name(sub))
                variants.add(normalize_name(p + ' ' + parts[i+1]))
                i += 2; continue
            except: pass
        if re.match(r'^[A-Za-z]$', p) and i+1 < len(parts) and re.match(r'^\d+$', parts[i+1]):
            sub = p + parts[i+1]
            variants.add(normalize_name(sub))
            i += 2; continue
        i += 1
    if len(parts) >= 2:
        variants.add(normalize_name(parts[0] + parts[1]))
    return variants

def _build_enhanced_gc_dist_index(gc_dists):
    enhanced = {}
    for key, val in gc_dists.items():
        variants = _extract_gc_name_variants(key)
        for v in variants:
            if v not in enhanced:
                enhanced[v] = val
    return enhanced

def _lookup_gc_dist_enhanced(cluster_name, gc_dists, enhanced_index=None):
    nn = normalize_name(cluster_name)
    for nt in [nn, cluster_name, cluster_name.lower(), cluster_name.upper()]:
        if nt in gc_dists:
            return gc_dists[nt]
    if enhanced_index is not None:
        query_variants = _extract_gc_name_variants(cluster_name)
        for qv in query_variants:
            if qv in enhanced_index:
                return enhanced_index[qv]
    if len(nn) >= 4:
        for key, val in gc_dists.items():
            nk = normalize_name(key)
            if len(nk) >= 4 and (nn in nk or nk in nn):
                return val
    return None, None
...
# ============================================================================
# APJ PAPER STYLE — V12b: Bold thick ticks, matched to reference image
# ============================================================================
def set_paper_style():
...
def process_gc_members(master, gc_dists, logger, epoch_mode='2016', eod=None):
    ...
    logger.info(f"\n{'─'*50}\nGLOBULAR CLUSTERS [{epoch_mode}] (V12b)\n{'─'*50}")
    ...
    enhanced_idx=_build_enhanced_gc_dist_index(gc_dists)
    logger.info(f"  Enhanced GC dist index: {len(enhanced_idx)} keys (from {len(gc_dists)} original)")
    results=[]
    for i,cn in enumerate(clusters):
        ...
        plot_rd, plot_rde = rd, rde
        if plot_rd is None or not np.isfinite(plot_rd):
            plot_rd, plot_rde = _lookup_gc_dist_enhanced(cn, gc_dists, enhanced_idx)
            if plot_rd is not None and np.isfinite(plot_rd):
                logger.info(f"    [V12b plot-only] Enhanced match found: d={plot_rd:.2f} kpc")
            else:
                plot_rd, plot_rde = None, None
        ...
        res=_make_result(cn,'GC',cdf,mdf,algo,cols,midx,rd,rde,rrv,rrve,
                                     md,memd,mrv,memrv,plxd,pra,pdec,ppmra,ppmdec,diag)
        res['plot_ref_dist']=plot_rd; res['plot_ref_dist_err']=plot_rde
        results.append(res)
        gcmod.collect()
    return results
...
def process_sgr_members(master, logger, epoch_mode='2016', eod=None):
    ...
    logger.info(f"\n{'─'*50}\nSGR STREAM [{epoch_mode}] (V12b Bayesian EM)\n{'─'*50}")
...
def process_dwg_members(master, logger, epoch_mode='2016', eod=None):
    ...
    logger.info(f"\n{'─'*50}\nDWARF GALAXIES [{epoch_mode}] (V12b)\n{'─'*50}")
...
# ============================================================================
# V12b: PLOTTING HELPERS — STYLE-MATCHED TO REFERENCE IMAGE
# ============================================================================
...
def _safe_kde(ax, d, bins, color, zorder=5):
    """V12b: KDE overlay — dashed line, matching reference image."""
    d2=d[np.isfinite(d)]
    if len(d2)<4: return
    try:
        kde=gaussian_kde(d2); x=np.linspace(bins[0],bins[-1],200)
        ax.plot(x,kde(x)*len(d2)*(bins[1]-bins[0]),color=color,
                ls=cfg.KDE_LS,alpha=cfg.KDE_ALPHA,lw=cfg.KDE_LW,zorder=zorder)
    except: pass

def _med_mad_box(ax, data, color, loc='upper right', prefix=''):
    """V12b: Med/MAD/N stat box — as in reference image."""
    ...
    ax.text(x,y,txt,transform=ax.transAxes,fontsize=8,fontweight='bold',ha=ha,va=va,family='monospace',
            bbox=dict(boxstyle='round,pad=0.3',facecolor='white',edgecolor=color,alpha=0.92,lw=1.5))

def _hist_step_kde(ax, data, color, label, br=None, nb=30, zorder=5, lw=None):
    """V12b: Step histogram + KDE overlay with zorder control."""
    d=_clean(data)
    if len(d)<3: return np.array([]),np.array([])
    if br is None:
        lo,hi=np.percentile(d,[0.5,99.5]); rng=max(hi-lo,1); br=[lo-0.1*rng,hi+0.1*rng]
    if lw is None: lw=cfg.STEP_LW_GREEN if color==cfg.COL_HIGHMEM else cfg.STEP_LW_BLUE
    c_,b_,_=ax.hist(d,bins=nb,range=br,alpha=0)
    kw={'where':'post','color':color,'lw':lw,'zorder':zorder}
    if label is not None: kw['label']=label
    ax.step(b_[:-1],c_,**kw)
    _safe_kde(ax,d,b_,color,zorder=zorder+1)
    # V12b: dotted median line (matching ref image)
    ax.axvline(np.median(d),color=color,ls=':',lw=2.0,alpha=0.9,zorder=zorder)
    return c_,b_
...
def _draw_median_mad_lines(ax, data, color, zorder=8):
    """V12b: DOTTED median + very faint MAD band with THIN DOTTED edge lines.
    Sleek classic style matching reference image."""
    ...
    if mad>0:
        # Very faint MAD band fill
        ax.axvspan(med-mad,med+mad,alpha=cfg.MAD_ALPHA,color=color,zorder=zorder-2)
        # Thin dotted edge lines (sleek classic)
        for edge in [med-mad, med+mad]:
            ax.axvline(edge,color=color,lw=cfg.MAD_EDGE_LW,alpha=cfg.MAD_EDGE_ALPHA,
                       ls=cfg.MAD_EDGE_LS,zorder=zorder-1)

def _draw_ref_band(ax, rv, re, zorder=20, unit='kpc'):
    """V12b: Reference band — solid black center line, very faint gray fill,
    DOTTED black edge lines. Matches reference image exactly."""
    if rv is None or not np.isfinite(rv): return
    ax.axvline(rv,color='black',lw=cfg.REF_LW,alpha=cfg.REF_ALPHA,
               zorder=zorder,label=f'Ref={rv:.1f} {unit}')
    if re is not None and np.isfinite(re) and re>0:
        # Faint gray fill
        ax.axvspan(rv-re,rv+re,alpha=cfg.REF_BAND_ALPHA,color='gray',zorder=zorder-2)
        # DOTTED black edge lines
        for edge in [rv-re, rv+re]:
            ax.axvline(edge,color='black',lw=cfg.REF_BAND_EDGE_LW,
                       alpha=cfg.REF_BAND_EDGE_ALPHA,ls=cfg.REF_BAND_EDGE_LS,
                       zorder=zorder-1)

def _draw_all_matched_hist(ax, data, br, nb=25, zorder=None):
    """V12b: Red all-matched histogram — visible dashed, behind green."""
    if zorder is None: zorder=cfg.ZORDER_RED
...
def _draw_all_matched_hist_pos(ax, data, br, nb=25, zorder=None):
    """V12b: Red all-matched for positive data (dist)."""
    if zorder is None: zorder=cfg.ZORDER_RED
...
# ============================================================================
# V12b: PLOT-ONLY REFERENCE DISTANCE RESOLVER
# ============================================================================
def _get_plot_ref(result):
    """V12b: Get best reference distance for plotting.
    For GCs, falls back to enhanced-matched plot_ref_dist if ref_dist is None.
    NOTE: GC ref distances come from literature (GC_dist.csv).
          OC ref distances come from parallax inversion.
          SGR ref distances come from distance bin centers.
          DW ref distances come from catalog metadata.
    Returns (rd, rde) for plotting only."""
    rd = result.get('ref_dist')
    rde = result.get('ref_dist_err')
    if (rd is None or not np.isfinite(rd)) and result.get('obj_type') == 'GC':
        prd = result.get('plot_ref_dist')
        prde = result.get('plot_ref_dist_err')
        if prd is not None and np.isfinite(prd):
            return prd, prde
    return rd, rde

# ============================================================================
# V12b: INDIVIDUAL PLOTS (4-panel: PM, Sky, Distance, RV)
# ============================================================================
def plot_individual_panels(mdf, name, ot, algo, rd=None, rde=None, rrv=None, rrve=None,
    ...
    # Layered: red(bottom) -> plx(mid) -> green(top) -> ref(topmost)
    _draw_all_matched_hist_pos(ax,alld,br,25,zorder=cfg.ZORDER_RED)
    if len(plxc)>=3:
        _hist_step_kde(ax,plxc,cfg.COL_PLX,f'Parallax dist (n={len(plxc)})',br,25,zorder=cfg.ZORDER_PLX)
        _draw_median_mad_lines(ax,plxc,cfg.COL_PLX,zorder=cfg.ZORDER_PLX+1)
        _med_mad_box(ax,plxc,cfg.COL_PLX,'upper left','Parallax ')
    if len(hd)>=3:
        _hist_step_kde(ax,hd,cfg.COL_HIGHMEM,f'P>{thr:.1f} (n={len(hd)})',br,25,zorder=cfg.ZORDER_GREEN)
        _draw_median_mad_lines(ax,hd,cfg.COL_HIGHMEM,zorder=cfg.ZORDER_GREEN+1)
        _med_mad_box(ax,hd,cfg.COL_HIGHMEM,'upper right',f'P>{thr:.1f} ')
    _draw_ref_band(ax,rd,rde,zorder=cfg.ZORDER_REF)
    # V12b: NO KS p-value text
    ...
    if len(hrc)>=3 or len(arc)>=3 or len(memrv_c)>=3:
        ...
        _draw_all_matched_hist(ax,allrv,br,25,zorder=cfg.ZORDER_RED)
        if len(memrv_c)>=3:
            _hist_step_kde(ax,memrv_c,cfg.COL_PLX,f'Member RV (n={len(memrv_c)})',br,25,zorder=cfg.ZORDER_PLX)
            _draw_median_mad_lines(ax,memrv_c,cfg.COL_PLX,zorder=cfg.ZORDER_PLX+1)
        if len(hrc)>=3:
            _hist_step_kde(ax,hrc,cfg.COL_HIGHMEM,f'P>{thr:.1f} (n={len(hrc)})',br,25,zorder=cfg.ZORDER_GREEN)
            _draw_median_mad_lines(ax,hrc,cfg.COL_HIGHMEM,zorder=cfg.ZORDER_GREEN+1)
            _med_mad_box(ax,hrc,cfg.COL_HIGHMEM,'upper right',f'P>{thr:.1f} ')
        _draw_ref_band(ax,rrv,rrve,zorder=cfg.ZORDER_REF,unit='km/s')
...
# ============================================================================
# V12b: SMART GOLD SAMPLE FILTER
# ============================================================================
def _gold_sample(all_results):
    thr=cfg.P_MEM_PLOT_THRESHOLD
    primary=cfg.SUMMARY_MIN_HIGH_PMEM
    relaxed=cfg.SUMMARY_MIN_HIGH_PMEM_RELAX
    min_per_type=cfg.SUMMARY_MIN_PER_TYPE
    scored=[]
    ...
    for otype,rlist in types_seen.items():
        ...
        if len(primary_pass)>=min_per_type: gold.extend(primary_pass)
        else:
...
# ============================================================================
# V12b: PAGINATED 6×6 SUMMARY PANELS — per-panel legends + stat boxes
# ============================================================================
def _paginated_summary(gold, panel_func, series_name, logger, out_dir, em):
    """V12b: Per-panel legends and stat boxes, bold ticks, no KS text."""
    import matplotlib.pyplot as plt
    ...
    _axis_labels={
        'Dist':  ('Distance (kpc)','Count'),
        'RV':    (r'RV (km s$^{-1}$)','Count'),
        'PM':    (r'$\mu_\alpha\cos\delta$ (mas yr$^{-1}$)',r'$\mu_\delta$ (mas yr$^{-1}$)'),
        'RADEC': ('RA (deg)','Dec (deg)'),
    }
    xlab,ylab=_axis_labels.get(series_name,('',''))
    for pg in range(npages):
        ...
        fig.suptitle(f'V12 {series_name} Summary ($P_{{mem}}$>{thr:.1f}) {et}  '
                     f'[page {pg+1}/{npages}]',fontsize=14,fontweight='bold',y=0.995)
        ...
        fig.text(0.5,0.005,xlab,ha='center',va='bottom',fontsize=13,fontweight='bold')
        fig.text(0.005,0.5,ylab,ha='left',va='center',rotation='vertical',fontsize=13,fontweight='bold')
...
# ── V12b: panel functions with per-panel legend + stat boxes ──

def _panel_dist(ax, r, thr, tc):
    mdf=r['matched_df']; ot=r['obj_type']; cn=r['cluster_name']
    rd, rde = _get_plot_ref(r)
    ...
    _draw_all_matched_hist_pos(ax,alld,br,20,zorder=cfg.ZORDER_RED)
    if len(plxc)>=3:
        _hist_step_kde(ax,plxc,cfg.COL_PLX,f'Parallax dist (n={len(plxc)})',br,20,zorder=cfg.ZORDER_PLX)
        _draw_median_mad_lines(ax,plxc,cfg.COL_PLX,zorder=cfg.ZORDER_PLX+1)
        _med_mad_box(ax,plxc,cfg.COL_PLX,'upper left','Parallax ')
    if len(hd)>=3:
        _hist_step_kde(ax,hd,cfg.COL_HIGHMEM,f'High-P (n={len(hd)})',br,20,zorder=cfg.ZORDER_GREEN)
        _draw_median_mad_lines(ax,hd,cfg.COL_HIGHMEM,zorder=cfg.ZORDER_GREEN+1)
        _med_mad_box(ax,hd,cfg.COL_HIGHMEM,'upper right',f'P>{thr:.1f} ')
    _draw_ref_band(ax,rd,rde,zorder=cfg.ZORDER_REF)
    ax.set_title(f'{ot}: {cn}',fontsize=9,fontweight='bold',color=tc.get(ot,'k'),pad=3)
    ax.tick_params(labelsize=7,which='both',direction='in',top=True,right=True)
    ax.tick_params(which='major',length=6,width=1.4)
    ax.tick_params(which='minor',length=3.5,width=0.9)
    ax.minorticks_on()
    ax.legend(fontsize=6,loc='center right',framealpha=0.85,edgecolor='gray',fancybox=False)
...
def _panel_rv(ax, r, thr, tc):
    ...
    if len(hrc)>=3 or len(arc)>=3 or len(memrv_c)>=3:
        br=_bins_range_with_ref([arc,hrc,memrv_c],rrv)
        _draw_all_matched_hist(ax,allrv,br,20,zorder=cfg.ZORDER_RED)
        if len(memrv_c)>=3:
            _hist_step_kde(ax,memrv_c,cfg.COL_PLX,None,br,20,zorder=cfg.ZORDER_PLX)
            _draw_median_mad_lines(ax,memrv_c,cfg.COL_PLX,zorder=cfg.ZORDER_PLX+1)
        if len(hrc)>=3:
            _hist_step_kde(ax,hrc,cfg.COL_HIGHMEM,None,br,20,zorder=cfg.ZORDER_GREEN)
            _draw_median_mad_lines(ax,hrc,cfg.COL_HIGHMEM,zorder=cfg.ZORDER_GREEN+1)
        _draw_ref_band(ax,rrv,rrve,zorder=cfg.ZORDER_REF,unit='km/s')
        vs=r['algo_info'].get('v_sys_recovered',np.nan)
        if np.isfinite(vs): ax.axvline(vs,color='darkblue',lw=1.5,ls='-.',alpha=0.7,zorder=15)
    else: ax.text(0.5,0.5,'No RV',ha='center',va='center',transform=ax.transAxes,fontsize=9,color='gray')
    ax.set_title(f'{ot}: {cn}',fontsize=9,fontweight='bold',color=tc.get(ot,'k'),pad=3)
    ax.tick_params(labelsize=7,which='both',direction='in',top=True,right=True)
    ax.tick_params(which='major',length=6,width=1.4)
    ax.tick_params(which='minor',length=3.5,width=0.9)
    ax.minorticks_on(); ax.grid(True,alpha=0.15,axis='y')
...
def _panel_pm(ax, r, thr, tc):
    ...
    ax.tick_params(which='major',length=6,width=1.4)
    ax.tick_params(which='minor',length=3.5,width=0.9)
    ax.minorticks_on(); ax.grid(True,alpha=0.15); ax.set_aspect('equal','datalim')
...
def generate_grand_summary(gold, logger, out_dir, em='2016'):
    ...
    col_labels=['Sky (RA,Dec)','Proper Motion','Distance','Radial Velocity']
    for ci,lab in enumerate(col_labels):
        axes[0,ci].annotate(lab,xy=(0.5,1.15),xycoords='axes fraction',
            ha='center',va='bottom',fontsize=12,fontweight='bold',family='serif')
    _legend_handles=[
        Line2D([0],[0],color=cfg.COL_MASTER,ls='--',lw=cfg.COL_MASTER_LW,alpha=max(cfg.COL_MASTER_ALPHA,0.6),label='All matched'),
        Line2D([0],[0],color=cfg.COL_HIGHMEM,lw=2.2,label=f'High $P_{{mem}}$ (>{thr:.1f})'),
        Line2D([0],[0],color=cfg.COL_PLX,lw=2.2,label='Parallax / Mem RV'),
        Line2D([0],[0],color='black',lw=cfg.REF_LW,alpha=cfg.REF_ALPHA,label='Reference'),
        Patch(facecolor='gray',alpha=0.15,edgecolor='black',linewidth=0.8,label='Ref band'),
    ]
    fig.suptitle(f'V12 Grand Summary ($P_{{mem}}$>{thr:.1f}) {et}',fontsize=18,fontweight='bold',y=1.0)
    fig.legend(handles=_legend_handles,loc='upper center',ncol=5,fontsize=10,
              frameon=True,framealpha=0.9,edgecolor='gray',
              bbox_to_anchor=(0.5,0.997),borderpad=0.3)
    plt.tight_layout(rect=[0,0,1,0.98])
    plt.savefig(os.path.join(out_dir,f'GRAND_SUMMARY.{cfg.SAVE_FORMAT}'),dpi=300,bbox_inches='tight'); plt.close()
    logger.info(f"  Saved: GRAND_SUMMARY ({n} objects x 4 cols)")

# ============================================================================
# V12b: PICKLE SAVE/LOAD FOR REPLOT MODE
# ============================================================================
def save_results_pickle(all_results, out_dir, em):
    """Save all_results to pickle for fast replot."""
    fp=os.path.join(out_dir,f'V12b_results_epoch{em}.pkl')
    # Strip non-serializable items (member_df as CSV, matched_df as CSV, etc.)
    # We save the full objects — pickle handles DataFrames fine
    with open(fp,'wb') as f:
        pickle.dump(all_results, f, protocol=pickle.HIGHEST_PROTOCOL)
    return fp

def load_results_pickle(out_dir, em):
    """Load all_results from pickle for replot."""
    fp=os.path.join(out_dir,f'V12b_results_epoch{em}.pkl')
...
def run_epoch_analysis(master, gc_dists, logger, em, eo_dir, ec_dir, skip_plots=False):
    ...
    # V12b: Save results as pickle for replot mode
    pkl_path=save_results_pickle(all_results, eo_dir, em)
...
def run_replot(logger, em, eo_dir):
    """V12b: Replot mode — load pickle and regenerate all plots."""
    logger.info(f"\n{'='*70}\nV12b REPLOT MODE — EPOCH {em}\n{'='*70}")
    all_results=load_results_pickle(eo_dir, em)
    if all_results is None:
        logger.error(f"No pickle found at {eo_dir}/V12b_results_epoch{em}.pkl — run full analysis first!")
        return
    logger.info(f"  Loaded {len(all_results)} results from pickle")
    os.makedirs(os.path.join(eo_dir,'individual_plots'),exist_ok=True)
    idir=os.path.join(eo_dir,'individual_plots')
    for r in all_results:
        if r['matched_df'] is None or r['n_matched']<cfg.MIN_STARS_FOR_ANALYSIS: continue
        try:
    ...
    generate_summary_plots(all_results,logger,eo_dir,em)
    logger.info(f"  REPLOT DONE for epoch {em}")

def parse_args():
    p=argparse.ArgumentParser(description='Adaptive Membership V12b-Plots')
    p.add_argument('--master',default=None,help='Master catalog (not needed for --replot)')
    ...
    p.add_argument('--replot',action='store_true',
                   help='Replot only: load saved pickle results, skip all analysis')
...
def main():
    ...
    log_path=args.log or os.path.join(cfg.OUTPUT_DIR,'adaptive_membership_v12b.log')
    logger=setup_logging(log_path)

    # ── V12b: REPLOT MODE ──
    if args.replot:
        logger.info("="*70)
        logger.info("V12b REPLOT MODE — Plots only from saved pickle")
        logger.info("="*70)
        for em in args.epochs:
            eo=os.path.join(cfg.OUTPUT_DIR,f'epoch{em}')
            run_replot(logger,em,eo)
        logger.info(f"\n{'='*70}\nV12b REPLOT COMPLETE\n{'='*70}")
        return

    # ── Full analysis mode ──
    if not args.master:
        logger.error("--master required for full analysis (use --replot for plot-only mode)")
        sys.exit(1)
    ...
    logger.info("="*70)
    logger.info("ADAPTIVE MEMBERSHIP ANALYSIS V12b-PLOTS")
    logger.info(f"  7-term: Spatial+PM+RV+Distance+CMD+[Fe/H]+logg")
    logger.info(f"  V12b: Dotted ref edges | Thin MAD bands | Per-panel stats")
    logger.info(f"         No KS text | Green on top | Pickle caching")
    logger.info(f"         Ref bands: GC=literature, OC=parallax, SGR=bin, DW=catalog")
