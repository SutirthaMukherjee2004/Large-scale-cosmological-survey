...
"""
================================================================================
ADAPTIVE MEMBERSHIP ANALYSIS V6 — CHUNK-LOADING + MEDIAN±MAD BANDS
================================================================================

V6 CHANGES over V5:
 1. Master catalog loaded from Entire_catalogue_chunk*.fits (glob pattern).
    Column mapping updated:  RA_final, DEC_final, pmra_final, pmdec_final,
    distance_final, Weighted_Avg_final, parallax_final, etc.
 2. RV column: Weighted_Avg_final (weighted average, non-outlier).
 3. When NO reference distance/RV is available → median±MAD bands shown for
    both master (red) and member (blue) distributions, in BOTH individual
    and summary plots.
 4. Yellow summary-stats panel REMOVED from individual plots → clean 2×2.
 5. Everything else (algorithms, processing pipelines, crossmatch logic)
    preserved from V5.
...
class Config:
    MASTER_CATALOG = None          # single file OR glob pattern
    ...
    COL_MASTER = '#DC143C'   # crimson-red
    COL_MEMBER = '#000080'   # navy blue
...
# ============================================================================
# MASTER CATALOG — V6: CHUNK LOADING
# ============================================================================
...
class MasterCatalog:
    ...
    def load(self, filepath, checkpoint_dir=None):
        """Load master catalog. `filepath` can be:
           - glob pattern  (e.g. '/path/to/Entire_catalogue_chunk*.fits')
           - directory      (looks for Entire_catalogue_chunk*.fits inside)
           - single .fits   (legacy mode)
        """
        from astropy.io import fits
        self.logger.info("=" * 70)
        self.logger.info("LOADING MASTER CATALOG (V6)")
        self.logger.info("=" * 70)
        ...
        for fi, fpath in enumerate(files):
            ...
            try:
                with fits.open(fpath, memmap=True) as hdul:
                    ...
                    # Distance: use distance_final directly as best_dist
                    ...
                    # RV: Weighted_Avg_final (primary) with fallbacks
                    ...
                    # Parallax: use parallax_final directly
                    ...
                    if plx_col in col_names:
                        ...
                        # Fallback: try stellar_params_est column 4
    ...
    def _resolve_files(self, filepath):
        """Resolve filepath to a list of FITS files."""
        # Glob pattern?
        ...
        if os.path.isdir(filepath):
            ...
            # Fallback: any FITS in directory
...
def _bins_range_with_ref(arrays, ref_val=None, positive_only=False):
    """Compute bins range that includes all data AND the reference value."""
...
def _add_mad_band(ax, data, color, alpha=0.15):
    """V6: Add MAD shaded band centred on median (when no reference exists)."""
    d = _clean(data)
    if len(d) < 3: return
    med = np.median(d)
...
# ============================================================================
# INDIVIDUAL PLOTS (V6: 2×2, no stats panel, median±MAD fallback)
# ============================================================================
...
def plot_individual_panels(matched_df, obj_name, obj_type, algo_info,
    ...
    # V6: always 2×2 layout (no stats panel)
    ...
    # ===== Panel 2: Distance (raw, no delta) =====
    ax = af[2]
    md_c = _clean(master_dist) if master_dist is not None and len(master_dist) > 0 else np.array([])
    md_c = md_c[(md_c > 0) & (md_c < 300)]
    memd_c = _clean(member_dist) if member_dist is not None and len(member_dist) > 0 else np.array([])
    memd_c = memd_c[(memd_c > 0) & (memd_c < 300)]
    br = _bins_range_with_ref([md_c, memd_c], ref_dist, positive_only=True)
    if len(md_c) >= 3:
        _hist_step_kde(ax, md_c, cfg.COL_MASTER, f'Master (n={len(md_c)})', br, 25)
        _med_mad_box(ax, md_c, cfg.COL_MASTER, 'upper right', 'Master ')
    if len(memd_c) >= 3:
        _hist_step_kde(ax, memd_c, cfg.COL_MEMBER, f'Member (n={len(memd_c)})', br, 25)
        _med_mad_box(ax, memd_c, cfg.COL_MEMBER, 'upper left', 'Member ')

    # V6: Reference line OR median±MAD bands
    if ref_dist is not None and np.isfinite(ref_dist):
        ...
        # No reference → show median±MAD for each distribution
        if len(md_c) >= 3:
            _add_mad_band(ax, md_c, cfg.COL_MASTER, alpha=0.12)
        if len(memd_c) >= 3:
            _add_mad_band(ax, memd_c, cfg.COL_MEMBER, alpha=0.12)
    ...
    ax.legend(fontsize=10, loc='best'); ax.grid(True, alpha=0.2, axis='y')

    # ===== Panel 3: RV =====
    ax = af[3]
    mrv_c = _clean(master_rv) if master_rv is not None and len(master_rv) > 0 else np.array([])
    mem_rv_c = _clean(member_rv) if member_rv is not None and len(member_rv) > 0 else np.array([])
    show_rv = len(mrv_c) >= 3 or len(mem_rv_c) >= 3

    if show_rv:
        br = _bins_range_with_ref([mrv_c, mem_rv_c], ref_rv)
        if len(mrv_c) >= 3:
            _hist_step_kde(ax, mrv_c, cfg.COL_MASTER, f'Master (n={len(mrv_c)})', br, 25)
            _med_mad_box(ax, mrv_c, cfg.COL_MASTER, 'upper right', 'Master ')
        if len(mem_rv_c) >= 3:
            _hist_step_kde(ax, mem_rv_c, cfg.COL_MEMBER, f'Member (n={len(mem_rv_c)})', br, 25)
        ...
        if ref_rv is not None and np.isfinite(ref_rv):
            ...
            if len(mrv_c) >= 3:
                _add_mad_band(ax, mrv_c, cfg.COL_MASTER, alpha=0.12)
            if len(mem_rv_c) >= 3:
                _add_mad_band(ax, mem_rv_c, cfg.COL_MEMBER, alpha=0.12)
...
def process_dwg_members(master, logger):
    """V5/V6: Standard crossmatch via RA_master/Dec_master."""
    if not cfg.DWG_MEMBERS_FILE or not os.path.exists(cfg.DWG_MEMBERS_FILE):
        return []
    logger.info("\n" + "-"*50 + "\nProcessing DWARF GALAXIES (V6: standard xmatch)...\n" + "-"*50)
    ...
    for i, gn in enumerate(gals):
        ...
        if ref_rv is not None:
            logger.info(f"  Ref RV (vlos_sys): {ref_rv:.1f} ± {ref_rv_err if ref_rv_err else 0:.1f} km/s")
        ...
        if len(midx) >= cfg.MIN_STARS_FOR_ANALYSIS:
            ...
            if rv_col in mm.columns:
                ...
                logger.warning(f"  RV column '{rv_col}' not found in member catalog")
...
# ============================================================================
# SUMMARY PLOTS (V6: median±MAD fallback, flexible grid)
# ============================================================================
...
def generate_summary_plots(all_results, logger):
    ...
    # ===== 3. DISTANCE SUMMARY (V6: median±MAD fallback) =====
    fig, axes = plt.subplots(nr, nc, figsize=(5*nc, 4.5*nr))
    if n == 1: axes = np.array([axes])
    af = axes.flatten()
    for i, r in enumerate(sr):
        ax = af[i]
        md_c = (_clean(r['master_dist'])
                if r['master_dist'] is not None and len(r['master_dist']) > 0
                else np.array([]))
        md_c = md_c[(md_c > 0) & (md_c < 300)]
        mem_c = (_clean(r['member_dist'])
                 if r['member_dist'] is not None and len(r['member_dist']) > 0
                 else np.array([]))
        mem_c = mem_c[(mem_c > 0) & (mem_c < 300)]
        rd = r.get('ref_dist'); rde = r.get('ref_dist_err')
        br = _bins_range_with_ref([md_c, mem_c], rd, positive_only=True)
        if len(md_c) >= 3:
            _hist_step_kde(ax, md_c, cfg.COL_MASTER, 'Master', br, 20)
            _med_mad_box(ax, md_c, cfg.COL_MASTER, 'upper right', 'M ')
        if len(mem_c) >= 3:
            _hist_step_kde(ax, mem_c, cfg.COL_MEMBER, 'Member', br, 20)
            _med_mad_box(ax, mem_c, cfg.COL_MEMBER, 'upper left', 'Mem ')
        ...
        if rd is not None and np.isfinite(rd):
            ...
            if len(md_c) >= 3:
                _add_mad_band(ax, md_c, cfg.COL_MASTER, alpha=0.10)
            if len(mem_c) >= 3:
                _add_mad_band(ax, mem_c, cfg.COL_MEMBER, alpha=0.10)
    ...
    for j in range(n, len(af)): af[j].set_visible(False)
    fig.suptitle('Distance Distribution Summary', fontsize=22, fontweight='bold')
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(cfg.OUTPUT_DIR, f'SUMMARY_Distance.{cfg.SAVE_FORMAT}'),
                dpi=cfg.PLOT_DPI); plt.close()
    logger.info("  Saved: SUMMARY_Distance")

    # ===== 4. RV SUMMARY (V6: median±MAD fallback) =====
    fig, axes = plt.subplots(nr, nc, figsize=(5*nc, 4.5*nr))
    if n == 1: axes = np.array([axes])
    af = axes.flatten()
    for i, r in enumerate(sr):
        ax = af[i]
        mrv = (_clean(r['master_rv'])
               if r['master_rv'] is not None and len(r['master_rv']) > 0
               else np.array([]))
        mem_rv = (_clean(r['member_rv'])
                  if r['member_rv'] is not None and len(r['member_rv']) > 0
                  else np.array([]))
        ...
        if not all_rv:
            ...
            br = _bins_range_with_ref([mrv, mem_rv], rrv)
            if len(mrv) >= 3:
                _hist_step_kde(ax, mrv, cfg.COL_MASTER,
                               f'Master (n={len(mrv)})', br, 25)
                _med_mad_box(ax, mrv, cfg.COL_MASTER, 'upper right', 'M ')
            ax.set_ylabel('Count (Master)', fontsize=13, fontweight='bold',
                          color=cfg.COL_MASTER)
            if len(mem_rv) >= 3:
                ax2 = ax.twinx()
                counts2, bins2, _ = ax2.hist(mem_rv, bins=25, range=br, alpha=0)
                ax2.step(bins2[:-1], counts2, where='post', color=cfg.COL_MEMBER,
                         linewidth=2.5, label=f'Member (n={len(mem_rv)})')
                _safe_kde(ax2, mem_rv, bins2, cfg.COL_MEMBER)
                ax2.axvline(np.median(mem_rv), color=cfg.COL_MEMBER, ls=':', lw=2.5)
                ax2.set_ylabel('Count (Member)', fontsize=13, fontweight='bold',
                               color=cfg.COL_MEMBER)
            ...
            if rrv is not None and np.isfinite(rrv):
                ...
                if len(mrv) >= 3:
                    _add_mad_band(ax, mrv, cfg.COL_MASTER, alpha=0.10)
                if len(mem_rv) >= 3:
                    _add_mad_band(ax, mem_rv, cfg.COL_MEMBER, alpha=0.10)
...
def parse_args():
    p = argparse.ArgumentParser(description='Adaptive Membership V6 (chunk loading)')
...
def main():
    ...
    logger = setup_logging(
        args.log or os.path.join(cfg.OUTPUT_DIR, 'adaptive_membership_v6.log'))
    logger.info("=" * 70)
    logger.info("ADAPTIVE MEMBERSHIP ANALYSIS V6")
    logger.info("  Chunk loading | Weighted_Avg_final | Median±MAD bands")
    logger.info("=" * 70)
