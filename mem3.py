...
def plot_individual_panels(mdf, name, ot, algo, rd=None, rde=None, rrv=None, rrve=None,
    ...
    if len(ad)>=3:
        ...
        ax.step(b_[:-1],c_,where='post',color=cfg.COL_MASTER,lw=1.5,alpha=0.5,ls='--',
    ...
    if len(plxc)>=3:
        ...
        _med_mad_box(ax,plxc,cfg.COL_PLX,'upper left','Blue ')
...
def generate_summary_plots(all_results, logger, eod=None, em='2016'):
    ...
    logger.info(f"\n{'='*70}\nV11 SUMMARY + GRAND PLOTS\n{'='*70}")
    gold=_gold_sample(all_results,cfg.SUMMARY_MIN_HIGH_PMEM)
    if not gold:
        logger.warning("No objects pass gold filter (>=10 green). Fallback to 3.")
        gold=_gold_sample(all_results,min_green=3)
