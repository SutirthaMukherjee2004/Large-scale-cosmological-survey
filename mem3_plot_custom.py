...
V11 over V10:
 ...
 5. All V10 preserved.
Custom plotting variant:
 - Navy-blue auxiliary/member overlays
 - Dashed median lines for green/navy
 - Slightly more opaque MAD bands
 - Paged (<=40/page) square-ish summary outputs
...
class Config:
    ...
    SUMMARY_PRIMARY_GREEN_MIN=15
    SUMMARY_FALLBACK_GREEN_MIN=10
    SUMMARY_MIN_PER_TYPE=5
    SUMMARY_MAX_PER_PAGE=40
    CMAP_PMEM='RdYlGn'; PLOT_DPI=150; SAVE_FORMAT='png'
    COL_MASTER='#DC143C'; COL_MEMBER='#000080'; COL_HIGHMEM='#006400'
    COL_PLX='#000080'  # Navy-blue auxiliary/member distribution
...
def _gc_direct_lookup_keys(name):
    """
    Direct (pre-normalization) lookup keys:
      - exact stripped name
      - case-insensitive stripped name
    """
...
def _gc_name_variants(name):
    ...
    map to the same reference-distance row.
    """
    ...
    s = s0.replace('–', '-').replace('—', '-').replace('−', '-')
    s = re.sub(r'\s+', ' ', s).strip()
    add(s0); add(s)
    add(s.replace(' ', '')); add(s.replace(' ', '_')); add(s.replace('_', ' '))
    add(s.replace('-', ' ')); add(s.replace('-', ''))

    for ch in [c for c in re.split(r'[|,;/]+', s) if c.strip()]:
        add(ch)
    ...
    for i, t in enumerate(tl):
        ...
        if m:
            ...
            for nn in {num, num2}:
                add(f"{pref} {nn}"); add(f"{pref}_{nn}"); add(f"{pref}{nn}")
...
def _gc_ref_lookup(cluster_name, gc_dists):
    """
    Return:
      (rd, rde, match_mode, matched_key)
    match_mode in {'direct','normalized','none'}
    """
    if isinstance(gc_dists, dict) and 'direct' in gc_dists and 'normalized' in gc_dists:
        direct_map = gc_dists.get('direct', {})
        norm_map = gc_dists.get('normalized', {})
    else:
        # Backward-compatible fallback (older normalized-only map format)
        direct_map = {}
        norm_map = gc_dists if isinstance(gc_dists, dict) else {}

    for k in _gc_direct_lookup_keys(cluster_name):
        if k in direct_map:
            rd, rde = direct_map[k]
            return rd, rde, 'direct', k
    for k in _gc_name_variants(cluster_name):
        if k in norm_map:
            rd, rde = norm_map[k]
            return rd, rde, 'normalized', k
    return (None, None, 'none', None)

def _report_gc_ref_coverage(cluster_names, gc_dists, logger):
    missing = []; n_direct = 0; n_norm = 0
    for cn in cluster_names:
        rd, _, mode, _ = _gc_ref_lookup(cn, gc_dists)
    ...
    logger.info(f"GC distance coverage: {n_all - n_mis}/{n_all} matched "
                f"(direct={n_direct}, normalized={n_norm})")
    if n_mis > 0:
...
def load_gc_reference_distances(fp, logger):
    ...
    for _,row in df.iterrows():
        ...
        if np.isfinite(dist):
            # Direct lookup: pre-normalization (exact + case-insensitive only)
            for k in _gc_direct_lookup_keys(name):
                gc_direct[k]=(dist,err)
            # Normalized lookup: flexible fallback for plotting-only references
            for k in _gc_name_variants(name):
                gc_norm[k]=(dist,err)
    logger.info(f"  Loaded GC refs: direct_keys={len(gc_direct)} | normalized_keys={len(gc_norm)}")
    return {'direct':gc_direct, 'normalized':gc_norm}
...
def _run_em_and_build(cdf, mdf, mm, cols, c_pmra, c_pmdec, rd, rde, ref_rv, ref_rv_err,
    ...
    rd_use = rd if use_ref_distance else None
    rde_use = rde if use_ref_distance else None
    dm_use = dist_mod if use_ref_distance else None
    mcmd=_build_cmd_kde_from_members(cdf,cols,dm_use); fcmd=_build_field_cmd_kde(mdf)
    kw=dict(pmra=mdf['pmra'].values,pmdec=mdf['pmdec'].values,cp=c_pmra,cd=c_pmdec,
        ...
        ref_dist=rd_use,ref_dist_err=rde_use,
        ...
        dist_modulus=dm_use,member_cmd_kde=mcmd,field_cmd_kde=fcmd,
...
def process_gc_members(master, gc_dists, logger, epoch_mode='2016', eod=None):
    ...
    for i,cn in enumerate(clusters):
        ...
        rd,rde,ref_mode,ref_key=_gc_ref_lookup(cn, gc_dists)
        use_ref_likelihood=(ref_mode=='direct')
        if rd is not None and np.isfinite(rd) and not use_ref_likelihood:
            logger.info(f"    GC ref via normalized key '{ref_key}' -> plot-only (no distance/CMD prior)")
        ...
        if len(midx)>=cfg.MIN_STARS_FOR_ANALYSIS:
            ...
            mdf,P,algo=_run_em_and_build(cdf,mdf,mm,cols,cpmra,cpmdec,rd,rde,rrv,rrve,dm,'GC',logger,
                                          ra_c=rac,dec_c=decc,use_ref_distance=use_ref_likelihood)
            algo['gc_ref_match_mode']=ref_mode
            ...
            diag=compute_diagnostics(mdf,P,rd if use_ref_likelihood else None,rrv,'GC',algo,logger)
...
def _draw_median_mad_lines(ax, data, color, zorder=8):
    ...
    if len(d)<3: return
    med=np.median(d); mad=median_abs_deviation(d,nan_policy='omit')
    ax.axvline(med,color=color,ls='--',lw=2.5,alpha=0.95,zorder=zorder)
    if mad>0: ax.axvspan(med-mad,med+mad,alpha=0.26,color=color,zorder=zorder-1)
...
def plot_individual_panels(mdf, name, ot, algo, rd=None, rde=None, rrv=None, rrve=None,
    ...
    if len(ad)>=3:
        c_,b_,_=ax.hist(ad,bins=25,range=br,alpha=0)
        ax.step(b_[:-1],c_,where='post',color=cfg.COL_MASTER,lw=1.5,alpha=0.28,ls='--',
            label=f'All matched (n={len(ad)})',zorder=3)
    if len(hd)>=3:
    ...
    if len(plxc)>=3:
        _hist_step_kde(ax,plxc,cfg.COL_PLX,f'1/plx (n={len(plxc)})',br,25)
        _draw_median_mad_lines(ax,plxc,cfg.COL_PLX,zorder=7)
        _med_mad_box(ax,plxc,cfg.COL_PLX,'upper left','Navy ')
    ...
    if len(hrc)>=3 or len(arc)>=3:
        ...
        if memrv is not None and len(memrv)>0:
            memr=_clean(np.asarray(memrv,dtype=float))
        br=_bins_range_with_ref([arc,hrc,memr],rrv)
        if len(arc)>=3:
            c_,b_,_=ax.hist(arc,bins=25,range=br,alpha=0)
            ax.step(b_[:-1],c_,where='post',color=cfg.COL_MASTER,lw=1.5,alpha=0.28,ls='--',
                label=f'All matched (n={len(arc)})',zorder=3)
        if len(hrc)>=3:
        ...
        if len(memr)>=3:
            _hist_step_kde(ax,memr,cfg.COL_PLX,f'Member RV (n={len(memr)})',br,25)
            _draw_median_mad_lines(ax,memr,cfg.COL_PLX,zorder=7)
            _med_mad_box(ax,memr,cfg.COL_PLX,'upper left','Navy ')
...
def _gold_sample(all_results, min_green=None):
    if min_green is None: min_green=cfg.SUMMARY_PRIMARY_GREEN_MIN
    ...
    key=lambda x: (type_order.index(x['obj_type']) if x['obj_type'] in type_order else 99,
                   -x.get('_n_hi',0), -x.get('n_matched',0))
    selected=[r for r in cands if r.get('_n_hi',0)>=min_green]

    # Ensure >=5 objects per type if possible; relax threshold to 10 for top-up
    seen={(r['obj_type'],str(r['cluster_name'])) for r in selected}
    for t in type_order:
        have=[r for r in selected if r['obj_type']==t]
        if len(have)>=cfg.SUMMARY_MIN_PER_TYPE:
            continue
        pool=[r for r in cands
              if r['obj_type']==t
              and r.get('_n_hi',0)>=cfg.SUMMARY_FALLBACK_GREEN_MIN
              and (r['obj_type'],str(r['cluster_name'])) not in seen]
        pool=sorted(pool,key=key)
        need=cfg.SUMMARY_MIN_PER_TYPE-len(have)
        for rr in pool[:need]:
            selected.append(rr)
            seen.add((rr['obj_type'],str(rr['cluster_name'])))

    if not selected:
        selected=[r for r in cands if r.get('_n_hi',0)>=cfg.SUMMARY_FALLBACK_GREEN_MIN]
    if not selected:
        selected=sorted(cands,key=key)[:cfg.SUMMARY_MAX_PER_PAGE]
...
def _summary_pages(gold, max_per_page=None):
    if max_per_page is None:
        max_per_page=cfg.SUMMARY_MAX_PER_PAGE
    for i0 in range(0,len(gold),max_per_page):
        yield (i0//max_per_page)+1, gold[i0:i0+max_per_page]
...
def generate_summary_radec(gold, logger, out_dir, em='2016'):
    import matplotlib.pyplot as plt
    if len(gold)==0: return
    tc={'GC':'#006400','OC':'#FF8C00','DW':'#00008B','SGR':'#8B0000'}
    et='[J2000->2016]' if em=='2000' else '[J2016]'
    for pi, page in _summary_pages(gold):
        n=len(page); nrows,ncols=_summary_grid(n)
        ...
        for j,r in enumerate(page):
            ...
            if pra is not None and pdec is not None:
                ...
                if np.sum(vm)>0:
                    ax.scatter(pra[vm],pdec[vm],c=cfg.COL_GRAY_BG,s=cfg.GRAY_BG_SIZE,
                               alpha=cfg.GRAY_BG_ALPHA,zorder=1,edgecolors='none')
            if 'P_mem' in mdf.columns:
                v=mdf['P_mem'].notna()
                ax.scatter(mdf.loc[v,'ra'],mdf.loc[v,'dec'],c=mdf.loc[v,'P_mem'],
                           cmap=cfg.CMAP_PMEM,s=15,edgecolors='k',lw=0.2,vmin=0,vmax=1,zorder=5)
            ax.set_xlabel('RA (deg)',fontsize=11,fontweight='bold')
            ax.set_ylabel('Dec (deg)',fontsize=11,fontweight='bold')
            ax.set_title(f'{ot}: {cn}',fontsize=11,fontweight='bold',color=tc.get(ot,'k'))
        ...
        fig.suptitle(f'V11 Summary — RA/Dec (P$_{{mem}}$>{cfg.P_MEM_PLOT_THRESHOLD:.1f}) {et} [page {pi}]',
                     fontsize=16,fontweight='bold')
        plt.tight_layout(rect=[0,0,1,0.96])
        of=os.path.join(out_dir,f'SUMMARY_RADEC_{pi}.{cfg.SAVE_FORMAT}')
        plt.savefig(of,dpi=cfg.PLOT_DPI); plt.close()
        logger.info(f"  Saved: {os.path.basename(of)}")
    ...
    for pi, page in _summary_pages(gold):
        ...
        for j,r in enumerate(page):
            ...
            if pp is not None and ppd is not None:
                ...
                if np.sum(vm)>0:
                    ax.scatter(pp[vm],ppd[vm],c=cfg.COL_GRAY_BG,s=cfg.GRAY_BG_SIZE,
                               alpha=cfg.GRAY_BG_ALPHA,zorder=1,edgecolors='none')
            if 'P_mem' in mdf.columns:
                v=mdf['P_mem'].notna()
            ...
            if algo.get('Sigma_cluster') and ot not in ['SGR','STREAM']:
                mu=algo['mu_cluster']; sig=np.array(algo['Sigma_cluster'])
                w,v2=np.linalg.eigh(sig); ang=np.degrees(np.arctan2(v2[1,0],v2[0,0]))
                ell=Ellipse(xy=mu,width=4*np.sqrt(max(w[0],0)),height=4*np.sqrt(max(w[1],0)),
                            angle=ang,fill=False,ec='lime',lw=2.3)
                ax.add_patch(ell)
            ax.set_xlabel(r'$\mu_\alpha\cos\delta$ (mas/yr)',fontsize=11,fontweight='bold')
            ax.set_ylabel(r'$\mu_\delta$ (mas/yr)',fontsize=11,fontweight='bold')
            ax.set_title(f'{ot}: {cn}',fontsize=11,fontweight='bold',color=tc.get(ot,'k'))
            ax.grid(True,alpha=0.2); ax.set_aspect('equal','datalim')
        for j in range(n,nrows*ncols): axes[j//ncols,j%ncols].set_visible(False)
        fig.suptitle(f'V11 Summary — PM (P$_{{mem}}$>{cfg.P_MEM_PLOT_THRESHOLD:.1f}) {et} [page {pi}]',
                     fontsize=16,fontweight='bold')
        plt.tight_layout(rect=[0,0,1,0.96])
        of=os.path.join(out_dir,f'SUMMARY_PM_{pi}.{cfg.SAVE_FORMAT}')
        plt.savefig(of,dpi=cfg.PLOT_DPI); plt.close()
        logger.info(f"  Saved: {os.path.basename(of)}")
...
def generate_summary_dist(gold, logger, out_dir, em='2016'):
    ...
    for pi, page in _summary_pages(gold):
        ...
        for j,r in enumerate(page):
            ...
            hd=_clean(hid); hd=hd[(hd>0)&(hd<300)]
            ad=_clean(alld); ad=ad[(ad>0)&(ad<300)]
            plxd=r.get('plx_dist',np.array([])); plxc=np.array([])
            if plxd is not None and len(plxd)>0:
                plxc=_clean(plxd); plxc=plxc[(plxc>0)&(plxc<300)]
            br=_bins_range_with_ref([ad,hd,plxc],rd,pos=True)
            if len(ad)>=3:
                c_,b_,_=ax.hist(ad,bins=20,range=br,alpha=0)
                ax.step(b_[:-1],c_,where='post',color=cfg.COL_MASTER,lw=1.0,alpha=0.28,ls='--',
                        label=f'All (n={len(ad)})',zorder=3)
            if len(hd)>=3:
            ...
            if dg.get('ks_dist_pval') is not None and np.isfinite(dg.get('ks_dist_pval')):
                pv=dg.get('ks_dist_pval')
                ax.text(0.03,0.03,f"KS p={pv:.2f}",transform=ax.transAxes,fontsize=8,
                        color='green' if pv>0.05 else 'red',fontweight='bold')
        ...
        for j in range(n,nrows*ncols): axes[j//ncols,j%ncols].set_visible(False)
        fig.suptitle(f'V11 Summary — Distance (P$_{{mem}}$>{thr:.1f}) {et} [page {pi}]',
                     fontsize=16,fontweight='bold')
        plt.tight_layout(rect=[0,0,1,0.96])
        of=os.path.join(out_dir,f'SUMMARY_Dist_{pi}.{cfg.SAVE_FORMAT}')
        plt.savefig(of,dpi=cfg.PLOT_DPI); plt.close()
        logger.info(f"  Saved: {os.path.basename(of)}")
...
def generate_summary_rv(gold, logger, out_dir, em='2016'):
    ...
    for pi, page in _summary_pages(gold):
        ...
        for j,r in enumerate(page):
            ...
            memrv=r.get('member_rv',np.array([])); mrc=np.array([])
            if memrv is not None and len(memrv)>0:
                mrc=_clean(np.asarray(memrv,dtype=float))
            if len(hrc)>=3 or len(arc)>=3 or len(mrc)>=3:
                br=_bins_range_with_ref([arc,hrc,mrc],rrv)
                if len(arc)>=3:
                    c_,b_,_=ax.hist(arc,bins=20,range=br,alpha=0)
                    ax.step(b_[:-1],c_,where='post',color=cfg.COL_MASTER,lw=1,alpha=0.28,ls='--',
                            label=f'All (n={len(arc)})',zorder=3)
                if len(hrc)>=3:
                    _hist_step_kde(ax,hrc,cfg.COL_HIGHMEM,f'P>{thr:.1f} (n={len(hrc)})',br,20)
                    _draw_median_mad_lines(ax,hrc,cfg.COL_HIGHMEM,zorder=8)
                if len(mrc)>=3:
                    _hist_step_kde(ax,mrc,cfg.COL_PLX,f'Member RV (n={len(mrc)})',br,20)
                    _draw_median_mad_lines(ax,mrc,cfg.COL_PLX,zorder=7)
                _draw_ref_band(ax,rrv,rrve,zorder=20)
                vs=r['algo_info'].get('v_sys_recovered',np.nan)
                if np.isfinite(vs):
                    ax.axvline(vs,color='darkblue',lw=2,ls='-.',alpha=0.8,zorder=15,label=f'$v_{{sys}}$={vs:.1f}')
                dg=r.get('diagnostics',{})
                if dg.get('ks_rv_pval') is not None and np.isfinite(dg.get('ks_rv_pval')):
                    pv=dg.get('ks_rv_pval')
                    ax.text(0.03,0.03,f"KS p={pv:.2f}",transform=ax.transAxes,fontsize=8,
                            color='green' if pv>0.05 else 'red',fontweight='bold')
            else:
                ax.text(0.5,0.5,'No RV',ha='center',va='center',transform=ax.transAxes,fontsize=12,color='gray')
            ax.set_xlabel('RV (km/s)',fontsize=11,fontweight='bold')
            ax.set_ylabel('N',fontsize=11,fontweight='bold')
            ax.set_title(f'{ot}: {cn}',fontsize=11,fontweight='bold',color=tc.get(ot,'k'))
            ax.legend(fontsize=7,loc='best'); ax.grid(True,alpha=0.2,axis='y')
        for j in range(n,nrows*ncols): axes[j//ncols,j%ncols].set_visible(False)
        fig.suptitle(f'V11 Summary — RV (P$_{{mem}}$>{thr:.1f}) {et} [page {pi}]',
                     fontsize=16,fontweight='bold')
        plt.tight_layout(rect=[0,0,1,0.96])
        of=os.path.join(out_dir,f'SUMMARY_RV_{pi}.{cfg.SAVE_FORMAT}')
        plt.savefig(of,dpi=cfg.PLOT_DPI); plt.close()
        logger.info(f"  Saved: {os.path.basename(of)}")
...
def generate_summary_plots(all_results, logger, eod=None, em='2016'):
    ...
    logger.info(f"\n{'='*70}\nV11 SUMMARY PLOTS (custom)\n{'='*70}")
    gold=_gold_sample(all_results,cfg.SUMMARY_PRIMARY_GREEN_MIN)
    if not gold:
        logger.warning(f"No objects pass primary filter (>= {cfg.SUMMARY_PRIMARY_GREEN_MIN} high-P members). "
                       f"Fallback to >= {cfg.SUMMARY_FALLBACK_GREEN_MIN}.")
        gold=_gold_sample(all_results,min_green=cfg.SUMMARY_FALLBACK_GREEN_MIN)
    ...
    tc=['GC','OC','DW','SGR']
    cnt={t:sum(1 for r in gold if r['obj_type']==t) for t in tc}
    logger.info(f"  Type counts: {cnt}")
    generate_summary_radec(gold,logger,out_dir,em)
    generate_summary_pm(gold,logger,out_dir,em)
    generate_summary_dist(gold,logger,out_dir,em)
    generate_summary_rv(gold,logger,out_dir,em)
