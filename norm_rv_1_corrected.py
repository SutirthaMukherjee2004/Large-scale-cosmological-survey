...
def bin_stat_with_counts(x, y, n_bins=40, p_lo=1, p_hi=99, min_count=10):
    """Like bin_stat but also returns counts per bin."""
    ok = np.isfinite(x) & np.isfinite(y)
    ...
    return np.array(bc), np.array(meds), np.array(mads), np.array(counts)
...
def phase7_survey_cal(cfg, survey_stars, gaia_cal, tch_results):
    ...
    if ckpt.exists():
        ...
        _d7 = load_ckpt(ckpt)
        # v9.4: clean stale non-dict entries (e.g. function refs from old checkpoints)
        for _k7 in list(_d7.keys()):
            if not isinstance(_d7[_k7], dict):
                log(f"  WARNING: removing non-dict entry '{_k7}' from Phase 7 checkpoint")
                del _d7[_k7]
            elif 'corrected_gaia_rv_fn' in _d7[_k7]:
                del _d7[_k7]['corrected_gaia_rv_fn']
        return _d7
...
def phase9_plots(cfg, dup_results, tch_results, gaia_cal, survey_cal,
    ...
    if 'GAIA' in sb:
        ...
        # ── Gaia cal before/after (Figs 9-11) — REDESIGNED v9.4 ─────────────
    # Per-survey panels with before/after calibration, polynomial fit
    # +/- Bevington CI, MAD bands, and N in bins below.
    # Layout matches paper Figs. 9/10/11.
    # ─────────────────────────────────────────────────────────────────────
    coeffs_all=gaia_cal.get('coefficients',{})
    ...
    for eq_name,coeffs in coeffs_all.items():
        ...
        log(f"  Gaia cal before/after (paper style): {eq_name} ({param})...")

        # Identify contributing surveys
        survey_names = [per_surv[k]['surv'] for k in keys]
        n_survs = len(survey_names)
        if n_survs == 0: continue

        # Figure layout: n_survs columns, 2 rows (main + N in bins) per column
        fig_w = max(4.0 * n_survs, 8)
        fig = plt.figure(figsize=(fig_w, 5.5))
        gs = gridspec.GridSpec(2, n_survs, height_ratios=[3, 1],
        ...
        for si, k in enumerate(keys):
            ...
            ax_main = fig.add_subplot(gs[0, si])
            ax_n    = fig.add_subplot(gs[1, si], sharex=ax_main)

            # ── Before calibration ────────────────────────────────────
            bc, meds, mads, counts = bin_stat_with_counts(
                xs_, ys_, n_bins=25, min_count=cfg.min_bin_count)
            if len(bc) == 0: continue

            ax_main.plot(bc, meds, 'o-', ms=4, lw=1.5, color='#2166ac',
                        alpha=0.85, label='Before', zorder=5)
            ax_main.fill_between(bc, meds-mads, meds+mads,
                                color='#2166ac', alpha=0.15, zorder=3)

            # ── After calibration ─────────────────────────────────────
            ys_cal = ys_ - np.polyval(coeffs, xs_)
            bc2, meds2, mads2, counts2 = bin_stat_with_counts(
                xs_, ys_cal, n_bins=25, min_count=cfg.min_bin_count)
            if len(bc2) > 0:
                ax_main.plot(bc2, meds2, 's-', ms=4, lw=1.5, color='#b2182b',
                            alpha=0.85, label='After', zorder=6)
                ax_main.fill_between(bc2, meds2-mads2, meds2+mads2,
                                    color='#b2182b', alpha=0.15, zorder=4)

            # ── Polynomial fit + Bevington CI ─────────────────────────
            x_fit = np.linspace(bc.min(), bc.max(), 200)
            y_fit = np.polyval(coeffs, x_fit)
            ax_main.plot(x_fit, y_fit, '--', color='#555555', lw=2.0,
                        alpha=0.8, zorder=7)
            if global_cov is not None:
                delta_f = np.array([_poly_err(coeffs, global_cov, xv) for xv in x_fit])
                ok_d = np.isfinite(delta_f)
                if np.any(ok_d):
                    ax_main.fill_between(x_fit[ok_d],
                                        (y_fit-delta_f)[ok_d], (y_fit+delta_f)[ok_d],
                                        color='gray', alpha=0.2, label='Fit CI', zorder=2)
                    # Error bars at binned positions
                    delta_bins = np.array([_poly_err(coeffs, global_cov, xv) for xv in bc])
                    ok_eb = np.isfinite(delta_bins)
                    if np.any(ok_eb):
                        y_fit_bins = np.polyval(coeffs, bc)
                        ax_main.errorbar(bc[ok_eb], y_fit_bins[ok_eb],
                                        yerr=delta_bins[ok_eb],
                                        fmt='none', ecolor='#555555', elinewidth=1.5,
                                        capsize=3, capthick=1.0, alpha=0.7, zorder=8)

            ax_main.axhline(0, color='black', ls='-', lw=0.8, alpha=0.5)
            ax_main.set_ylabel(r'$\Delta$RV (km/s)', fontsize=10)
            ax_main.set_ylim(-3, 3)
            ax_main.set_title(sn, fontsize=11, fontweight='bold')
            ...
            # ── N in bins ─────────────────────────────────────────────
            if len(counts) > 0:
                ax_n.plot(bc, counts, 'k.-', ms=3, lw=1)
                ax_n.set_yscale('log')
                ax_n.set_ylabel('N', fontsize=9)
                ax_n.set_xlabel(param, fontsize=10)
                ax_n.grid(True, alpha=0.25, ls='--')
            else:
                ax_n.set_xlabel(param, fontsize=10)

        fig.suptitle(f'Gaia Calibration {eq_name}: $\\Delta$RV vs {param}'
                     f' (Paper Figs. 9-11)\n'
                     f'Blue=Before | Red=After | Dashed=Fit $\\pm$ Bevington CI',
                     fontsize=11, y=1.02)
        fig.tight_layout()
        for e in ['png','pdf']: fig.savefig(out/f'gaia_cal_{param}.{e}',
                                            dpi=300, bbox_inches='tight')
        plt.close(fig)
        log(f"    gaia_cal_{param} saved ({n_survs} survey panels).")

    # ── Fig 12: Survey calibration — REDESIGNED v9.4 ─────────────────────
    # Per-survey before/after panels for each parameter, with N in bins.
    # Matches paper Fig. B.1 style.
    # ──────────────────────────────────────────────────────────────────────
    log("  Fig 12: Survey calibration — per-survey before/after panels...")
    param_list=['Teff','logg','FeH','SNR','RV','Gmag']
    param_labels = {'Teff': r'$T_{\rm eff}$ (K)', 'logg': r'$\log g$ (dex)',
                    'FeH': '[Fe/H] (dex)', 'SNR': 'S/N',
                    'RV': 'RV (km/s)', 'Gmag': 'G mag'}

    for surv, sres in sorted(survey_cal.items()):
        if not isinstance(sres, dict): continue
        if 'diag' not in sres or 'fits' not in sres: continue
        df_diag = sres['diag']
        if len(df_diag) < 50: continue

        # Determine which split applies and compute correction
        fits_dict = sres.get('fits', {})
        if not fits_dict: continue

        # For each star, compute the predicted correction from Eq.8
        drv_before = df_diag['drv'].values.copy()
        drv_after  = drv_before.copy()

        for split_name, fdata in fits_dict.items():
            feat_names = fdata['feat_names']
            coeffs_eq8 = fdata['coeffs']

            # Build feature matrix for this split
            # Determine which rows belong to this split
            sl = split_name.lower()
            if 'dwarf' in sl:
                mask_split = df_diag['logg'].values > 3.5 if 'logg' in df_diag.columns else np.ones(len(df_diag), bool)
            elif 'giant' in sl:
                mask_split = df_diag['logg'].values <= 3.5 if 'logg' in df_diag.columns else np.ones(len(df_diag), bool)
            elif 'cool' in sl:
                mask_split = df_diag['Teff'].values < 6200 if 'Teff' in df_diag.columns else np.ones(len(df_diag), bool)
            elif 'hot' in sl:
                mask_split = df_diag['Teff'].values >= 6200 if 'Teff' in df_diag.columns else np.ones(len(df_diag), bool)
            else:
                mask_split = np.ones(len(df_diag), bool)
            mask_split = mask_split & np.isfinite(mask_split)

            for idx_row in np.where(mask_split)[0]:
                feat_vals = {
                    'Teff':      df_diag['Teff'].iloc[idx_row] if 'Teff' in df_diag.columns else 0.0,
                    'Teff2':     (df_diag['Teff'].iloc[idx_row]**2) if 'Teff' in df_diag.columns else 0.0,
                    'logg':      df_diag['logg'].iloc[idx_row] if 'logg' in df_diag.columns else 0.0,
                    'FeH':       df_diag['FeH'].iloc[idx_row] if 'FeH' in df_diag.columns else 0.0,
                    'SNR':       df_diag['SNR'].iloc[idx_row] if 'SNR' in df_diag.columns else 0.0,
                    'RV':        df_diag['RV'].iloc[idx_row] if 'RV' in df_diag.columns else 0.0,
                    'intercept': 1.0,
                }
                correction = sum(
                    float(c) * float(feat_vals.get(fn, 0.0))
                    for c, fn in zip(coeffs_eq8, feat_names)
                    if np.isfinite(c) and np.isfinite(feat_vals.get(fn, 0.0))
                )
                drv_after[idx_row] = drv_before[idx_row] - correction

        # Determine which parameters to plot (need enough valid data)
        avail_params = []
        for pname in param_list:
            if pname in df_diag.columns and df_diag[pname].notna().sum() > 50:
                avail_params.append(pname)
            elif pname == 'RV' and 'RV' in df_diag.columns:
                avail_params.append(pname)
        if not avail_params: continue

        n_params = len(avail_params)
        n_cols = min(n_params, 3)
        n_rows_grid = (n_params + n_cols - 1) // n_cols

        fig = plt.figure(figsize=(5.5 * n_cols, 4.5 * n_rows_grid))
        gs_outer = gridspec.GridSpec(n_rows_grid, n_cols, hspace=0.45, wspace=0.30)

        for pi, pname in enumerate(avail_params):
            row_g = pi // n_cols
            col_g = pi % n_cols
            gs_inner = gridspec.GridSpecFromSubplotSpec(
                2, 1, subplot_spec=gs_outer[row_g, col_g],
                height_ratios=[3, 1], hspace=0.08)
            ax_main = fig.add_subplot(gs_inner[0])
            ax_n    = fig.add_subplot(gs_inner[1], sharex=ax_main)

            if pname in df_diag.columns:
                x = df_diag[pname].values.astype(np.float64)
            else:
                continue

            # Before calibration
            bc, meds, mads, counts = bin_stat_with_counts(
                x, drv_before, n_bins=25, min_count=cfg.min_bin_count)
            if len(bc) == 0: continue

            ax_main.plot(bc, meds, 'o-', ms=3, lw=1.5, color='#2166ac',
            ...
            # After calibration
            bc2, meds2, mads2, counts2 = bin_stat_with_counts(
                x, drv_after, n_bins=25, min_count=cfg.min_bin_count)
            if len(bc2) > 0:
                ax_main.plot(bc2, meds2, 's-', ms=3, lw=1.5, color='#b2182b',
                            alpha=0.85, label='After')
                ax_main.fill_between(bc2, meds2-mads2, meds2+mads2,
                                    color='#b2182b', alpha=0.12)

            ax_main.axhline(0, color='black', ls='-', lw=0.8, alpha=0.5)
            ax_main.set_ylabel(r'$\Delta$RV (km/s)', fontsize=9)
            xlabel = param_labels.get(pname, pname)
            ax_main.set_title(f'{pname}', fontsize=10)
            ax_main.set_ylim(-3, 3)
            ax_main.legend(fontsize=7, loc='upper right')
            ax_main.grid(True, alpha=0.25, ls='--')
            plt.setp(ax_main.get_xticklabels(), visible=False)
            ...
            # N in bins
            if len(counts) > 0:
                ax_n.plot(bc, counts, 'k.-', ms=2, lw=0.8)
                ax_n.set_yscale('log')
                ax_n.set_ylabel('N', fontsize=8)
                ax_n.set_xlabel(xlabel, fontsize=9)
                ax_n.grid(True, alpha=0.25, ls='--')

        fig.suptitle(f'Survey Calibration (Eq.8): {surv}\\n'
                     f'Blue=Before | Red=After correction'
                     f'  (N={len(df_diag):,})',
                     fontsize=12, y=1.01)
        fig.tight_layout()
        for e in ['png','pdf']:
            fig.savefig(out/f'fig12_{surv}.{e}', dpi=300, bbox_inches='tight')
        plt.close(fig)
        log(f"    fig12_{surv} saved ({len(avail_params)} parameters).")

    # Also generate the combined overview (all surveys, after calibration only)
    log("  Fig 12 combined: Calibrated DRV vs 6 params (all surveys)...")
    ...
    for pi,pname in enumerate(param_list):
        ...
        ax.set_title(f'Calibrated $\\Delta$RV vs {pname}',fontsize=11)
        ax.legend(fontsize=8); ax.grid(True,alpha=0.3,ls='--'); ax.set_ylim(-5,5)
    fig.suptitle('Survey Calibration: Calibrated $\\Delta$RV vs parameters (Paper Fig. 12)',fontsize=13)
    fig.tight_layout()
